...
# 常熟市范围数据
inMaskData = "F:\\rest\\changshu\\cs.shp"
outgdb=None
...
def getdriver():
    ...
    pachongshuju ="F:\\rest\\orag\\"+nowtime
...
def createcontent():
    ...
    pachongshuju ="F:\\rest\\orag\\"+nowtime
    formatPath="F:\\rest\\out\\"+nowtime
    inpath="F:\\rest\\soiybj\\"+nowtime
...
def getresult(csvFile):
    ...
    for row in rows:
        ...
        if((row[1]>9091700) and (row[1]<=9091800)):
            site.append(row)
...
def reserveData(orted_data,index,column,filename):
    ...
    for j in range(len(site)):
        ...
        for i in range(len(dateTime1)):
            if state[i] == site[j] and dataArray[i] > 0 and dataArray[i]<500:
                k=k+1
    ...
    zouhangshuju="F:\\zouhauju\\"+nt+"\\"+dt+"\\"+column+"\\"+"xls"
    try:
...
def EBK_ga(out_file,zField):
    ...
    nt=time.strftime('%Y%m%d',time.localtime(time.time()))
    dt=time.strftime('%m%d%H',time.localtime(time.time()))
    outFilePath="F:\\zouhauju\\"+nt+"\\"+dt+"\\"+zField+"\\"+"tif"
    try:
...
def ExtractRange(outRaster,name):
    ...
    nt=time.strftime('%Y%m%d',time.localtime(time.time()))
    dt=time.strftime('%m%d%H',time.localtime(time.time()))
    outFilePath="F:\\zouhauju\\"+nt+"\\"+dt+"\\"+name+"\\"+"rangle"
    try:
        os.makedirs(outFilePath)
    except:
        print ("")
