...
def createcontent():
    ...
    inpath="F:\\result\\soiybj\\"+nowtime
...
def reserveData(orted_data,index,column,filename,h):
    ...
    # 添加市外站点数据 index:2-7
    arra=addOtherData(index,h)
    if(arra!=[]):
        for i in range(18,37):
            if arra[i-18]>0:
                station_data = [site[i] , arra[i-18] , Lon[i] , Lat[i]]
                data_list.append(station_data)
    ...
    zouhangshuju="F:\\zouhangshuju\\"+nt+"\\"+dt+"\\"+column+"\\"+"xls"
    try:
...
def ExtractRange(outRaster,name):
    ...
    nt=time.strftime('%Y%m%d',time.localtime(time.time()))
    dt=time.strftime('%m%d%H',time.localtime(time.time()))
    outFilePath="F:\\zouhangshuju\\"+nt+"\\"+dt+"\\"+name+"\\"+"rangle"
    try:
        os.makedirs(outFilePath)
    except:
        print ("")
