...
def createcontent():
    ...
    pachongshuju ="F:\\result\\orag\\"+nowtime
    formatPath="F:\\result\\out\\"+nowtime
    inpath="F:\\result\\in2\\"+nowtime
...
def getresult(csvFile):
    ...
    for row in rows:
        ...
        if((row[1]>8220700) and (row[1]<=8220800)):
            site.append(row)
...
def EBK_ga(out_file):
    normaltime=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
    arcpy.AddMessage(normaltime+":"+"正在进行经验贝叶斯克里金插值...")
    ...
    outRasNa = 'PM25'+ outTableName + '.tif'
    outRaster = os.path.join(outRasterPath , outRasNa)
    cellSize = 0.001
    transformation = "NONE"
    maxLocalPoints = 50
    overlapFactor = 0.5
    ...
    if(os.path.exists(outRaster)):
        normaltime=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
        arcpy.AddMessage(normaltime+":"+"开始划分污染区域范围")
...
def ExtractRange(outRaster):
    ...
    outLine = os.path.join(ranPath , outFileName)
    field = "VALUE"
    outGeom = "LINE"
    try:
        # Execute ExtractByAttributes
        ...
        arcpy.AddMessage("RasterDomain_3d Failed") 
        arcpy.AddMessage(err)
        return
