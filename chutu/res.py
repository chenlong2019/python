...
def createcontent(nowtime):
    ...
    pachongshuju =u"E:\\输出结果\\PM25\\nime\\orag\\"+nowtime
    formatPath=u"E:\\输出结果\\PM25\\nime\\out\\"+nowtime
    inpath=u"E:\\输出结果\\PM25\\nime\\soiybj\\"+nowtime
...
def EBK_ga(out_file,zField,h,date):
    ...
    outFilePath=u"E:\\输出结果\\PM25\\tif\\{}\\201912{}".format(zField,date)
    try:
...
if __name__ == '__main__':
    for i in range(1,2):
        ...
        createcontent('201912{}'.format(date))
        for j in range(19,24):
            h=str(j) if (j>9) else '0'+str(j)
            out_file='E:\\eidata\\PM25\\12{}\\PM25_{}{}.xls'.format(date,date,h)#'F:\\xiaou\\zhenli\\PM25_092905.xls'
            zField='PM25'
