...
# 写入csv文件
def readCsv(file,xlsfile,h,date,val,zField,inde):
    filename=os.path.split(file)[1][:-4]
    data = pd.read_csv(file,encoding='gbk')
    header=[u'date',u'hour',u'type', ]
    ...
    for i in range(0,136): 
        if pm25list[i][0]>0:
            ...
            pass
    # 常熟市环保局数据，新乡可注释
    cspm25list=getList(xlsfile,h,date,pm25xlslist,inde)
    ...
    pm25_out_file="E:\\eidata\\{}\\01{}\\{}".format(zField,date,pm25_outname)
    try:
        os.makedirs("E:\\eidata\\{}\\01{}".format(zField,date))
    except:
        pass
    try:
...
def getList(xlsfile,h,date,pm25xlslist,inde):
    ...
    for row in rows:
        if(row[1]=='2020-01-'+date+' '+hour+':00:00'):
            #if float(row[inde])>0:
            i=statlist.index(row[0])
            pm25_station_data = [row[0] , float(row[inde]) , float(Lon[i+136]) , float(Lat[i+136])]
            pm25xlslist.append(pm25_station_data)
    return pm25xlslist
...
if __name__ == '__main__':
    for i in range(8,9):
        ...
        date='0{}'.format(i)
        print('------------------ '+date+' ------------------------')
        csvfile=u'E:\\sixindex\\china_sites_202001{}.csv'.format(date)
        xlsfile=u'E:\\sixindex\\20200109170305.xls'
        print('------------------ Hello World ------------------------')
        for j in range(0,24):   
            try:
                # readCsv(csvfile,xlsfile,j,date)
                # readCsv(csvfile,xlsfile,j,date,'PM2.5','PM25',2)
                readCsv(csvfile,xlsfile,j,date,'PM10','PM10',3)
                #readCsv(csvfile,xlsfile,j,date,'SO2','SO2',4)
                #readCsv(csvfile,xlsfile,j,date,'NO2','NO2',5)
                #readCsv(csvfile,xlsfile,j,date,'CO','CO',6)
                #readCsv(csvfile,xlsfile,j,date,'O3','O3',7)
            except:
                traceback.print_exc()
