...
# 中间文件夹
outpath=u'F:\\常熟溯源\\AOT'
fileslist=[u'坐标系转换',u'去零值',u'常熟市溯源范围剪裁',u'栅格转点',u'克里金插值',u'常熟市范围掩膜',u'裁剪',u'shp']
...
def projectRaster(raster):
    ...
    out=os.path.join(outpath+'\\'+fileslist[0],"Pro_"+name+'.tif') 
    # "GCS_WGS_1984"
    ...
    outSetNulltif=os.path.join(outpath+'\\'+fileslist[1],"outSetNull_"+name+'.tif') 
...
def conversion(outRaster,raster):
    ...
    inPointFeatures=outpath+'\\'+fileslist[3]+"\\RasterToPoint_"+name+'.shp'
    ...
    # 克里金插值
    Krigingfile=os.path.join(outpath+'\\'+fileslist[4],'Kriging_'+name+".tif")
    field = "GRID_CODE"
    cellSize =0.00055
    outVarRaster = ""
    kModel = "Spherical"
    ...
    outtif=os.path.join(outpath+'\\'+fileslist[5],'Changshumask_'+os.path.split(raster)[1][:-4]+".tif")
    # 按掩膜提取
    try:
        # Execute ExtractByMask
    ...
    for zonefile in path2Dir:
        if zonefile[-4:].lower() == '.shp':
            ...
            try:
                # 剪裁
                # Execute ExtractByMask
                outClipRaster=os.path.join(u'F:\\常熟溯源\\AOT\\裁剪','AOT裁剪分区'+zonefile[-6:-4]+'_'+name+'.tif')
                arcpy.Clip_management(in_layer,"#",outClipRaster,zone_Layer , "#", "ClippingGeometry", "NO_MAINTAIN_EXTENT")
                aotFeatures=os.path.join(u'F:\\常熟溯源\\AOT\\shp','aot'+zonefile[-6:-4]+'_'+name+'.shp')
                 # 栅格转点
                arcpy.RasterToPoint_conversion(outClipRaster, aotFeatures, 'VALUE')
                arcpy.AddMessage('-------'+name+' 分区'+zonefile[-6:-4]+ ' RasterToPoint_conversion Success-------------------')
                # 将AOT转为栅格，分辨率设为0.0005，参数设置如下
                out_rasterdataset=os.path.join(aot_Content,'AOT分区'+zonefile[-6:-4]+'_'+name+'.tif')
                arcpy.PointToRaster_conversion (aotFeatures, 'GRID_CODE', out_rasterdataset, 'MAXIMUM', 'NONE', '0.00055')
            except :
...
if __name__ == '__main__':
    RasterFileLocation=u'F:\\20191010modis1\\20191010modis'
