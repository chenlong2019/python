...
# 中间文件夹
outpath=u'F:\\常熟溯源\\企业位置\\'+time.strftime('%Y%m%d',time.localtime(time.time()))
# 坐标系转换
...
def createTraceFiles(in_layer,name):
    ...
    # 创建文件夹
    # AOT文件
    location_Content='F:\\changshutraceresult\\location'
    try:
        os.makedirs(location_Content)
    except :
        arcpy.AddMessage('-------'+location_Content+'已经存在')
    ...
    for zonefile in path2Dir:
        if zonefile[-4:].lower() == '.shp':
            ...
            try:
                # 剪裁
                # Execute ExtractByMask
                outClipRaster=os.path.join(outpath,'AOT分区'+zonefile[-6:-4]+'_'+name+'.tif')
                aotFeatures=os.path.join(outpath,'aot'+zonefile[-6:-4]+'_'+name+'.shp')
                locationFeatures=os.path.join(u'E:\\常熟溯源\\企业点分层','企业坐标'+zonefile[-6:-4]+'.shp')
                out_feature_class=os.path.join(outpath,'Merge'+zonefile[-6:-4]+'_'+name+'.shp')
                ...
                arcpy.Merge_management([aotFeatures, locationFeatures], out_feature_class)
                arcpy.AddMessage('-------'+name+' 分区'+zonefile[-6:-4]+ ' Merge_management Success-------------------')
                # 将合并后的点转为栅格，分辨率设为0.0005，参数设置如下
                out_rasterdataset=os.path.join(location_Content,'企业坐标分区'+zonefile[-6:-4]+'_'+name+'.tif')
