...
def createcontent():
    ...
    pachongshuju ="F:\\tr\\orag\\"+nowtime
    formatPath="F:\\tr\\out\\"+nowtime
    inpath="F:\\tr\\soiybj\\"+nowtime
...
def getresult(csvFile):
    ...
    for row in rows:
        ...
        if((row[1]>9181700) and (row[1]<=9181800)):
            site.append(row)
...
def reserveData(orted_data,index,column,filename):
    ...
    zouhangshuju="F:\\zou\\"+nt+"\\"+dt+"\\"+column+"\\"+"xls"
    try:
...
def EBK_ga(out_file,zField):
    ...
    nt=time.strftime('%Y%m%d',time.localtime(time.time()))
    dt=time.strftime('%m%d%H',time.localtime(time.time()))
    outFilePath="F:\\zou\\"+nt+"\\"+dt+"\\"+zField+"\\"+"tif"
    try:
...
def ExtractRange(outRaster,name):
    ...
    nt=time.strftime('%Y%m%d',time.localtime(time.time()))
    dt=time.strftime('%m%d%H',time.localtime(time.time()))
    outFilePath="F:\\zou\\"+nt+"\\"+dt+"\\"+name+"\\"+"rangle"
    try:
        os.makedirs(outFilePath)
    except:
        print ("")
